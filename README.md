...
The variable 'unit' is assigned input(). In the parenthesis we have the strings which asks us whether the temperature is in Celsius or Fahrenheit. User input should be one of those measurements: C or F
Next we have the variable 'temp' assigned float() - not int()- after which we have another function; input() In these parentheses we have the string which tells us to enter the temperature.
...
Next line is also indented: the function print() the f-string which tells you what the temperature in Celsius is.
...
The next line starts with the function print() followed by the string; "This is a loop for temperature conversion."
